...
FileHandler.prototype.watchForUpdates = function()
{
    var self = this;

    // Start with new session file, since we'll be adding to it.
    if (fs.existsSync(sessionStatsFile)) {
        fs.unlinkSync(sessionStatsFile);
    }

    fs.watchFile(tfDataFile, function (current, previous) {
        var rawStats, snapshot, sessionStats, matchStats;
        ...
        sessionStats = self.getAlreadyCompiledSessionStats() || {};

        tfStatsParser.addMatchStatsToSessionStats(matchStats, sessionStats);

        self.writeSnapshotFile();
        self.writeSessionStatsFile(sessionStats);
    });
};
