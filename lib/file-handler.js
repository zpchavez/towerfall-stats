...
FileHandler.prototype.getRawStatsSync = function()
{
    ...
    xmlParser.parseString(rawData, function(err, result) {
        ...
        if (result === null) {
            var error = new Error('File was empty');
...
FileHandler.prototype.watchForUpdates = function(callback)
{
    ...
    watcher.on('change', function (path, stats) {
        ...
        try {
            ...
            if (e.name !== 'InvalidFileError') {
                console.log(e.stack);
            }
            // InvalidFileError thrown if the file was not fully written yet.
            // That's okay; another change event will fire when the file is complete.
            return;
        }
